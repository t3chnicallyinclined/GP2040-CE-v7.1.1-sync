# GP2040-CE v0.7.12 - Tap Sync Window
...
This fork of GP2040-CE adds a **tap sync window**, a small buffer at the firmware level that groups near-simultaneous button presses so they always arrive together. It's a simple idea: if two buttons are pressed within a few milliseconds of each other, hold both until they can be reported at the same time. That's it.
...
## Why It Replaces the Debounce (and Why That's Fine)
...
Quick background on what debounce does. When a mechanical switch closes, the metal contacts physically bounce against each other for a brief period (generally under 5ms for standard microswitches like those in Sanwa, Seimitsu, Cherry, and Kailh buttons). That bounce creates rapid on/off signals. Without filtering, the controller could read those bounces as separate presses. Traditional debounce waits for the signal to stabilize before registering the input.

So if the sync window is designed for grouping simultaneous presses, how does it handle bounce? It comes down to one line in the code:

```
sync_new &= raw;
```

Every cycle, the firmware checks that every buffered press is still physically held on the GPIO pin. If a button bounces open mid-buffer, that line drops it. When the contact settles back closed, it gets re-captured. By the time the 8ms window expires, the bounce has long settled and only the stable press survives to be committed. The bounce noise gets continuously cleaned out.

For releases, the logic is similar. Releases pass through instantly (important for charge moves). If a release bounces and the contact briefly closes again, that bounce gets picked up as a "new press" and enters the buffer. But `sync_new &= raw` keeps checking: if the button is physically released (which it will be once the bounce settles), the phantom press gets cleaned out before the window expires. Nothing false reaches the game.

So the sync window handles bounce, but not because "8ms is longer than bounce" (that's what I originally thought, and it's an oversimplification). It handles it because the code continuously validates buffered presses against the actual physical state of the buttons, every single cycle.
...
- I'm not an electrical engineer. This is my best understanding of how the code interacts with switch bounce. If something feels off, stock GP2040-CE firmware is always there to flash back.
...
A pre-built `.uf2` is available on the [Releases page](https://github.com/t3chnicallyinclined/GP2040-CE-SyncWindow/releases).
